<script lang="ts">
    ...
    import {
        ...
        isMobileView,
        isUserLogged,
    } from "../viewModel/GroupListViewModel";
    ...
    let onSelect = (event) => control.select(event.detail.group);
    let onGroupAdd = (event) => control.add(event.detail.group);
...
<style>
    ...
    #groups-list {
        ...
        overflow: hidden;
        transition: 0.12s cubic-bezier(0.1, 0.9, 0.2, 1);
    }
    #groups-list.closed {
    ...
    :global(#user-button .person-picture) {
        margin-top: 2px;
    }
    :global(#user-button > * > *) {
